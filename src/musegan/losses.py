...
def get_adv_losses(discriminator_real_outputs, discriminator_fake_outputs,
    ...
    if kind == 'classic':
        ...
        loss_fn = relativistic_gan_losses
...
def relativistic_gan_losses(discriminator_real_outputs, discriminator_fake_outputs):
    """
    modified by xy
    Return the relativistic GAN losses for the generator and the discriminator.
    """
    discriminator_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=discriminator_real_outputs - discriminator_fake_outputs,
        labels=tf.ones_like(discriminator_fake_outputs)))

    generator_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=discriminator_fake_outputs - discriminator_real_outputs,
        labels=tf.ones_like(discriminator_fake_outputs)))
    return generator_loss, discriminator_loss
